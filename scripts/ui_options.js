...
function createBubbleChart1(error, countries, continentNames, clusterType) {
  ...
  if(clusterType == "Geo_Cluster") {
    cluster_domain = d3.set(countries.map(function(country) { return country.Geo_Cluster; }));
  } else if(clusterType == "direction") {
    cluster_domain = d3.set(countries.map(function(country) { return country.Direction_Cluster; }));
  } else if(clusterType == "time") {
    cluster_domain = d3.set(countries.map(function(country) { return country.Time_Cluster; }));
  }
  
  var continentColorScale = d3.scaleOrdinal(d3.schemeCategory10)
        .domain(cluster_domain.values());
  ...
  function toggleContinentKey(showContinentKey) {
    ...
    function createContinentKey() {
      var keyWidth = keyElementWidth * cluster_domain.values().length;
...
function continentForceX(d) {
    ...
    function createPopulationForces() {
      var continentNamesDomain = cluster_domain.values().map(function(Geo_Cluster) {
        return continentNames[Geo_Cluster];
      });
      ...
      return {
        x: d3.forceX(function(d) {
            return populationScaleX(continentNames[d.Geo_Cluster]) + centerCirclesInScaleBandOffset;
          }).strength(forceStrength),
        y: d3.forceY(function(d) {
